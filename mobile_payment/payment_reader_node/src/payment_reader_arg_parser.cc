...
PaymentReaderArgParser::PaymentReaderArgParser(int argc, char **argv)
{
    error_t parse_error = 0;

    argp_options_.push_back(kCertsFolderPathOption);
    argp_options_.push_back({0});

    memset(&arg_parser_, 0, sizeof(arg_parser_));
    arg_parser_.options = &argp_options_[0];
    arg_parser_.parser = &PaymentReaderArgParser::ArgpParseOption;
...
error_t
PaymentReaderArgParser::ArgpParseOption(int option_key, char *option_value,
                                        struct argp_state *state)
{
    error_t retval = 0;
    PaymentReaderInputArgs* in_args = (PaymentReaderInputArgs*) state->input;

    switch (option_key)
    {
        case kCertsFolderPathOptionKey:
            {
                in_args->certs_folder_path = option_value;
                break;
            }
        case ARGP_KEY_ARG:
            {
...
PaymentReaderArgParser::GetInputArgs(void)
{
    return input_args_;
}
